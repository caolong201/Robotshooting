...
namespace VSX.UniversalVehicleCombat.Mechs
{
    ...
    public class AIController : MechInput
    {
        ...
        {
            if (!base.Initialize(inputTarget)) return false;

            mechCharacterController = inputTarget.GetComponent<RigidbodyCharacterController>();
            mechGimbalController = inputTarget.GetComponent<GimballedVehicleController>();

            triggerablesManager = inputTarget.GetComponent<TriggerablesManager>();
            weapons = inputTarget.GetComponent<Weapons>();

            if (mechCharacterController != null && mechGimbalController != null)
            {

                moveTargetPosition = inputTarget.transform.position;

                Damageable[] damageables = inputTarget.transform.GetComponentsInChildren<Damageable>();
