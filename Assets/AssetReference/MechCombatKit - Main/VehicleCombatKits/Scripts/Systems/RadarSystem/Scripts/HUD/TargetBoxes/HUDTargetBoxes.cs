...
namespace VSX.UniversalVehicleCombat.Radar
{
    ...
    public class HUDTargetBoxes : HUDComponent
    {
        ...
        protected virtual void ShowTargetBox(Tracker tracker, Trackable trackable, HUDTargetBox targetBox)
        {
            ...
            if (!string.IsNullOrEmpty(PlayerAutoFire.Instance.HitEnemyName) && PlayerAutoFire.Instance.HitEnemyName.Equals(trackable.name))
            {
                isHitEnemy = true;
                targetBox.SetColor(Color.red);
            }
            else
            {
                isHitEnemy = false;
            ...
            if (!isInView)
            {
                switch (targetBoxOffScreenMode)
                {
                    case TargetBoxOffScreenMode.ClampedToBorder:
                        ...
                        if(!isHitEnemy) UIManager.Instance.ShowTextFindEnemies(true);
                        else
                        {
                            UIManager.Instance.ShowTextFindEnemies(false);
                        }
                        break;
            ...
            //         {
            //             Debug.LogError("Color " + teamColors[i].color + "  # " + trackable.Team.name);
