...
namespace VSX.UniversalVehicleCombat.Radar
{
    ...
    {
        [Tooltip("The number of trackers that may be updated in a single frame.")] [SerializeField]
        protected int trackerUpdatesPerFrame = 1;

        [Tooltip("Trackers to add to the group on start.")] [SerializeField]
        protected List<Tracker> startingTrackers = new List<Tracker>();

        [Tooltip("Game agents to link - vehicles they enter will have their Trackers added.")] [SerializeField]
        protected List<GameAgent> gameAgents = new List<GameAgent>();

        [Tooltip("Spawners to link - spawned vehicles will have their Trackers added.")] [SerializeField]
        ...
        protected virtual void Awake()
        {
            ...
            {
                if (!gameAgent.gameObject.activeSelf) continue;
        ...
        //custom code
        public void SetupEnemiesPerStage(int stage, int wave)
        {
            var configStage = SaveDataManager.Instance.GetCurrentStageConfig();
            if (configStage != null)
            {
                var currWave = SaveDataManager.Instance.GetCurrentWaveConfig();
                if (currWave != null)
                {
                    for (int i = 0; i < gameAgents.Count; ++i)
                    {
                        gameAgents[i].Vehicle.SetHealth(currWave.enemies[i].health, currWave.enemies[i].health);
                        gameAgents[i].Vehicle.GetComponent<RigidbodyCharacterController>().WalkSpeed = currWave.enemies[i].speed;
                    }
                }
            }
        }
