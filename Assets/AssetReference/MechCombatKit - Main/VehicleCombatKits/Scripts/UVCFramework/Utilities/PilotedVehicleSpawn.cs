...
namespace VSX.UniversalVehicleCombat
{
    ...
    public class PilotedVehicleSpawn : MonoBehaviour
    {
        ...
        [Tooltip("Whether to respawn after a specified time after the vehicle is destroyed.")] [SerializeField]
        protected bool respawnAfterTime = true;

        [Tooltip("How long to wait after the vehicle is destroyed to respawn.")] [SerializeField]
        protected float respawnTime = 3;

        protected float respawnStartTime;

        [Tooltip("Whether to use the Pool Manager to create the pilot and vehicle.")] [SerializeField]
        ...
        [Tooltip("Whether to parent the pilot to the spawn transform.")] [SerializeField]
        protected bool parentPilotToSpawnTransform = true;

        protected GameAgent spawnedPilot;

        public GameAgent Pilot
        {
            get { return spawnedPilot; }
        }

        [SerializeField] protected bool specifyLoadout = false;

        [SerializeField] protected List<int> loadout = new List<int>();

        [SerializeField] protected LoadoutItems loadoutItems;
        ...
        [Tooltip(
            "Whether to instantiate the vehicle. Be sure to set this to True if the vehicle reference is a prefab.")]
        [SerializeField]
        protected bool instantiateVehicle = true;

        [Tooltip("Whether to parent the vehicle to the spawn transform.")] [SerializeField]
        ...
        [Header("Events")] public UnityEvent onSpawned;
        ...
        public virtual void Spawn()
        {
            ...
            if (instantiatePilot)
            {
                ...
                {
                    spawnedPilot = PoolManager.Instance
                        .Get(pilotReference.gameObject, Vector3.zero, Quaternion.identity,
                            parentPilotToSpawnTransform ? spawnTransform : null).GetComponent<GameAgent>();
                }
                else
                {
                    if (!created)
            ...
            if (instantiateVehicle)
            {
                if (usePool && PoolManager.Instance != null)
                {
                    spawnedVehicle = PoolManager.Instance.Get(vehicleReference.gameObject, spawnTransform.position,
                            spawnTransform.rotation, parentVehicleToSpawnTransform ? spawnTransform : null)
                ...
                else
                {
                    if (!created)
                    {
                        spawnedVehicle = Instantiate(vehicleReference,
                            parentVehicleToSpawnTransform ? spawnTransform : null);
                        spawnedVehicle.transform.localPosition = Vector3.zero;
                        spawnedVehicle.transform.localRotation = Quaternion.identity;
                        spawnedVehicle.name = "Enemy_" + id;
                        id++;
                    }
                }
            }
